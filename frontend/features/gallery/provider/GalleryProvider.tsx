...
export function GalleryProvider(props: ParentProps<IGalleryProviderProps>) {
  ...
  DEBUG_INFO_LABEL("gallery", "current gallery item is", currentItemIndex())

  const goToNextItem = () => {
    if (currentItemIndex() === entries().length) {
      setShouldDisableNextBtn(true)
      DEBUG_INFO_LABEL("gallery", "next button disabled")
      return
    }
  ...
  const goToPrevItem = () => {
    if (currentItemIndex() === 0) {
      ...
      DEBUG_INFO_LABEL("gallery", "prev button disabled")
      return
    }
