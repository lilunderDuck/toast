...
export function createColumnsManager(
  ...
  const updateColumn: IColumnsManager["updateData$"] = (
    ...
    console.assert(previousData, `[table] Could not find column id: "${columnId}". Weird stuff may happen.`)
    let newData = data(previousData as any)

    console.log("[table] New data", newData)

    setColumns(columnArray.replace$(it => it.key === columnId, newData))

    console.log("[table] Updated additional data for", columnId, "->", columnArray.find$(it => it.key === columnId)[0])
    onDataUpdate()
  } 
